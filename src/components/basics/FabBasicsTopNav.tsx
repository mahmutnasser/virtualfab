...
export const FabBasicsTopNav: React.FC<FabBasicsTopNavProps> = ({
  ...
  return (
    <header
      role="banner"
      className={`min-h-16 bg-white border-b border-[#DCE5F2] px-2 sm:px-4 md:px-8 flex items-center justify-between gap-2 text-slate-800 shrink-0 select-none z-30 shadow-[0_1px_3px_rgba(0,0,0,0.03)] sticky top-0 ${className}`.trim()}
    >
      {/* Brand & Module Title */}
      ...
      <nav aria-label="Explore sections" className="flex items-center gap-1 rounded-xl bg-[#F2F7FF] p-1 border border-[#DCE5F2]">
        <button type="button" onClick={onNavigateToHome} className="inline-flex min-h-[44px] items-center rounded-lg px-2 sm:px-3 text-xs sm:text-sm font-semibold text-[#145DB4] hover:bg-white focus-visible:outline focus-visible:outline-2 focus-visible:outline-[#166FE5]">
          Home
        </button>
        ...
        <button
          ...
          className="inline-flex min-h-[44px] items-center gap-1.5 px-2.5 py-1.5 sm:px-4 sm:py-2 rounded-lg bg-[#166FE5] hover:bg-[#145DB4] text-white font-body text-xs sm:text-sm font-semibold transition-all shadow-sm hover:shadow cursor-pointer focus-visible:outline focus-visible:outline-2 focus-visible:outline-[#166FE5] focus-visible:outline-offset-2"
        >
          <svg className="hidden sm:block w-4 h-4 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5" aria-hidden="true">
